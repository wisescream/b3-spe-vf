# SIEM Stack - The All In One Multi Honeypot Platform

![SIEM Stack]
SIEM Stack is the all in one, optionally distributed, multiarch (amd64, arm64) honeypot plattform, supporting 20+ honeypots and countless visualization options using the Elastic Stack, animated live attack maps and lots of security tools to further improve the deception experience.
<br><br>

# TL;DR
1. Meet the [system requirements](#system-requirements). The SIEM Stack installation needs at least 8-16 GB RAM, 128 GB free disk space as well as a working (outgoing non-filtered) internet connection.
...
<!-- TOC -->
- [SIEM Stack - The All In One Multi Honeypot Platform](#SIEM Stack---the-all-in-one-multi-honeypot-platform)
- [TL;DR](#tldr)
...
- [Installation](#installation)
  ...
  - [Get and install SIEM Stack](#get-and-install-SIEM Stack)
  - [macOS \& Windows](#macos--windows)
  - [Installation Types](#installation-types)
    - [Standard / Hive](#standard--hive)
    - [Distributed](#distributed)
  - [Uninstall SIEM Stack](#uninstall-SIEM Stack)
...
- [Remote Access and Tools](#remote-access-and-tools)
  - [SSH](#ssh)
  - [SIEM Stack Landing Page](#SIEM Stack-landing-page)
...
- [Configuration](#configuration)
  - [SIEM Stack Config File](#SIEM Stack-config-file)
  - [Customize SIEM Stack Honeypots and Services](#customize-SIEM Stack-honeypots-and-services)
- [Maintenance](#maintenance)
  ...
  - [Known Issues](#known-issues)
    ...
    - [SIEM Stack Networking Fails](#SIEM Stack-networking-fails)
  - [Start SIEM Stack](#start-SIEM Stack)
  - [Stop SIEM Stack](#stop-SIEM Stack)
  - [SIEM Stack Data Folder](#SIEM Stack-data-folder)
  - [Log Persistence](#log-persistence)
  - [Factory Reset](#factory-reset)
  - [Show Containers](#show-containers)
  - [Blackhole](#blackhole)
  - [Add Users to Nginx (SIEM Stack WebUI)](#add-users-to-nginx-SIEM Stack-webui)
...
# Disclaimer
- You install and run SIEM Stack within your responsibility. Choose your deployment wisely as a system compromise can never be ruled out.
...
# Technical Concept
SIEM Stack's main components have been moved into the `tpotinit` Docker image allowing SIEM Stack to now support multiple Linux distributions, even macOS and Windows (although both limited to the feature set of Docker Desktop). SIEM Stack uses [docker](https://www.docker.com/) and [docker compose](https://docs.docker.com/compose/) to reach its goal of running as many honeypots and tools as possible simultaneously and thus utilizing the host's hardware to its maximum.
<br><br>

## Honeypots and Tools
- SIEM Stack offers docker images for the following honeypots:<br>
...
* [Elastic Stack](https://www.elastic.co/videos) to beautifully visualize all the events captured by SIEM Stack.
...
* [SIEM Stack-Attack-Map](https://github.com/t3chn0m4g3/SIEM Stack-attack-map) a beautifully animated attack map for SIEM Stack.
...
The source code and configuration files are fully stored in the SIEM Stack GitHub repository. The docker images are built and preconfigured for the SIEM Stack environment. 
...
## Services
SIEM Stack offers a number of services which are basically divided into five groups:
...
3. Tools
    * NGINX provides secure remote access (reverse proxy) to Kibana, CyberChef, Elasticvue, GeoIP AttackMap, Spiderfoot and allows for SIEM Stack sensors to securely transmit event data to the SIEM Stack hive.
    * CyberChef a web app for encryption, encoding, compression and data analysis.
    * Elasticvue a web front end for browsing and interacting with an Elasticsearch cluster.
    * SIEM Stack Attack Map a beautifully animated attack map for SIEM Stack.
...
## User Types
During the installation and during the usage of SIEM Stack there are two different types of accounts you will be working with. Make sure you know the differences of the different account types, since it is **by far** the most common reason for authentication errors.

| Service          | Account Type | Username / Group | Description                                                        |
| :--------------- | :----------- | :--------------- | :----------------------------------------------------------------- |
| SSH              | OS           | `<OS_USERNAME>`  | The user you chose during the installation of the OS.              |
| Nginx            | BasicAuth    | `<WEB_USER>`     | `<web_user>` you chose during the installation of SIEM Stack.           |
| CyberChef        | BasicAuth    | `<WEB_USER>`     | `<web_user>` you chose during the installation of SIEM Stack.           |
| Elasticvue       | BasicAuth    | `<WEB_USER>`     | `<web_user>` you chose during the installation of SIEM Stack.           |
| Geoip Attack Map | BasicAuth    | `<WEB_USER>`     | `<web_user>` you chose during the installation of SIEM Stack.           |
| Spiderfoot       | BasicAuth    | `<WEB_USER>`     | `<web_user>` you chose during the installation of SIEM Stack.           |
| SIEM Stack            | OS           | `tpot`           | `tpot` this user / group is always reserved by the SIEM Stack services. |
| SIEM Stack Logs       | BasicAuth    | `<LS_WEB_USER>`  | `LS_WEB_USER` are automatically managed.                           |
...
## Running in a VM
All of the [supported Linux distro images](#choose-your-distro) will run in a VM which means SIEM Stack will just run fine. The following were tested / reported to work:
...
- If you want to use a wifi card as a primary NIC for SIEM Stack, please be aware that not all network interface drivers support all wireless cards. In VirtualBox e.g. you have to choose the *"MT SERVER"* model of the NIC.
<br><br>

## Running on Hardware
SIEM Stack is only limited by the hardware support of the [supported Linux distro images](#choose-your-distro). It is recommended to check the HCL (hardware compatibility list) and test the supported distros with SIEM Stack before investing in dedicated hardware.
<br><br>

## Running in a Cloud
SIEM Stack is tested on and known to run on ...
* Telekom OTC using the post install method
... others may work, but remain untested. 

Some users report working installations on other clouds and hosters, i.e. Azure and GCP. Hardware requirements may be different. If you are unsure you should research [issues](https://github.com/telekom-security/tpotce/issues) and [discussions](https://github.com/telekom-security/tpotce/discussions) and run some functional tests. With SIEM Stack 24.04.0 and forward we made sure to remove settings that were known to interfere with cloud based installations. 
<br><br>

## Required Ports
Besides the ports generally needed by the OS, i.e. obtaining a DHCP lease, DNS, etc. SIEM Stack will require the following ports for incoming / outgoing connections. Review the [SIEM Stack Architecture](#technical-architecture) for a visual representation. Also some ports will show up as duplicates, which is fine since used in different editions.

| Port                                                                                                                                  | Protocol | Direction | Description                                                                                         |
| :------------------------------------------------------------------------------------------------------------------------------------ | :------- | :-------- | :-------------------------------------------------------------------------------------------------- |
| 80, 443                                                                                                                               | tcp      | outgoing  | SIEM Stack Management: Install, Updates, Logs (i.e. OS, GitHub, DockerHub, Sicherheitstacho, etc.        |
| 11434                                                                                                                                 | tcp      | outgoing  | LLM based honeypots: Access your Ollama installation                                                |
| 64294                                                                                                                                 | tcp      | incoming  | SIEM Stack Management: Sensor data transmission to hive (through NGINX reverse proxy) to 127.0.0.1:64305 |
| 64295                                                                                                                                 | tcp      | incoming  | SIEM Stack Management: Access to SSH                                                                     |
| 64297                                                                                                                                 | tcp      | incoming  | SIEM Stack Management Access to NGINX reverse proxy                                                      |
...
## LLM-Based Honeypots
We think LLM-Based Honeypots mark the **beginning** of a game change for the deception / honeypot field. Consequently, starting with the release of **SIEM Stack 24.04.1**, two LLM-based honeypots, **Beelzebub** and **Galah**, have been introduced. These honeypots require an installation of **Ollama**, which needs to be configured in the [SIEM Stack configuration file](#SIEM Stack-config-file). You can also adjust the settings in this file for **ChatGPT** support, but note that changes will also be required in the docker compose file (`~/tpotce/compose/llm.yml`) to accommodate these adjustments.<br><br>
...
### ChatGPT
ChatGPT support for these honeypots will remain untested in relation to SIEM Stack.
<br><br>

# System Placement
It is recommended to get yourself familiar with how SIEM Stack and the honeypots work before you start exposing towards the internet. For a quickstart run a SIEM Stack installation in a virtual machine.
<br><br>
Once you are familiar with how things work you should choose a network you suspect intruders in or from (i.e. the internet). Otherwise SIEM Stack will most likely not capture any attacks (unless you want to prove a point)! For starters it is recommended to put SIEM Stack in an unfiltered zone, where all TCP and UDP traffic is forwarded to SIEM Stack's network interface. To avoid probing for SIEM Stack's management ports you should put SIEM Stack behind a firewall and forward all TCP / UDP traffic in the port range of 1-64000 to SIEM Stack while allowing access to ports > 64000 only from trusted IPs and / or only expose the [ports](#required-ports) relevant to your use-case. If you wish to catch malware traffic on unknown ports you should not limit the ports you forward since glutton and honeytrap dynamically bind any TCP port that is not occupied by other honeypot daemons and thus give you a better representation of the risks your setup is exposed to.
<br><br>

# Installation
[Download](#choose-your-distro) one of the [supported Linux distro images](#choose-your-distro), follow the [TL;DR](#tldr) instructions or `git clone` the SIEM Stack repository and run the installer `~/tpotce/install.sh`. Running SIEM Stack on top of a running and supported Linux system is possible, but a clean installation is recommended to avoid port conflicts with running services. The SIEM Stack installer will require direct access to the internet as described [here](#required-ports).
<br><br>
...
3. **Never** install a graphical desktop environment such as Gnome or KDE. SIEM Stack will fail to work with it due to port conflicts. 
...
3. Run the installer as non-root: `$ ./install.sh`:
   * ⚠️ ***Depending on your Linux distribution of choice the installer will:***
     ...
     * Display open ports on the host (compare with SIEM Stack [required](https://github.com/telekom-security/tpotce#required-ports) ports)
     * Add and enable `tpot.service` to `/etc/systemd/system` so SIEM Stack can automatically start and stop
...
## macOS & Windows
Sometimes it is just nice if you can spin up a SIEM Stack instance on macOS or Windows, i.e. for development, testing or just the fun of it. As Docker Desktop is rather limited not all honeypot types or SIEM Stack features are supported. Also remember, by default the macOS and Windows firewall are blocking access from remote, so testing is limited to the host. For production it is recommended to run SIEM Stack on [Linux](#choose-your-distro).<br>
...
6. Adjust the `.env` file by changing `TPOT_OSTYPE=linux` to either `mac` or `win`:
   ...
   TPOT_OSTYPE=mac
   ```
7. You have to ensure on your own there are no port conflicts keeping SIEM Stack from starting up.
8. Start SIEM Stack: `docker compose up` or `docker compose up -d` if you want SIEM Stack to run in the background.
9. Stop SIEM Stack: `CTRL-C` (it if was running in the foreground) and / or `docker compose down -v` to stop SIEM Stack entirely.

## Installation Types

### Standard / Hive
With SIEM Stack Standard / Hive all services, tools, honeypots, etc. will be installed on to a single host which also serves as a Hive endpoint. Make sure to meet the [system requirements](#system-requirements). You can adjust `~/tpotce/docker-compose.yml` to your personal use-case or create your very own configuration using `~/tpotce/compose/customizer.py` for a tailored SIEM Stack experience to your needs.
Once the installation is finished you can proceed to [First Start](#first-start).
<br><br>

### Distributed
The distributed version of SIEM Stack requires at least two hosts
- the SIEM Stack **Hive**, the standard installation of SIEM Stack (install this first!),
- and a SIEM Stack **Sensor**, which will host only the honeypots, some tools and transmit log data to the **Hive**.
...
## Uninstall SIEM Stack
Uninstallation of SIEM Stack is only available on the [supported Linux distros](#choose-your-distro).<br>
To uninstall SIEM Stack run `~/tpotce/uninstall.sh` and follow the uninstaller instructions, you will have to enter your password at least once.<br>
Once the uninstall is finished reboot the machine `sudo reboot`
<br><br>

# First Start
Once the SIEM Stack Installer successfully finishes, the system needs to be rebooted (`sudo reboot`). Once rebooted you can log into the system using the user you setup during the installation of the system. Logins are according to the [User Types](#user-types):
...
You can also login from your browser and access the SIEM Stack WebUI and tools: `https://<your.ip>:64297`
- user: **[`<WEB_USER>`]**
- pass: **[password]**
<br><br>
...
The SIEM Stack configuration file (`.env`) does allow to disable the SSL verification for logstash connections from **Sensor** to the **Hive** by setting `LS_SSL_VERIFICATION=none`. For security reasons this is only recommended for lab or test environments.<br><br>
...
### Removing Sensors
Identify the `TPOT_HIVE_USER` ENV on the Sensor in the `$HOME/tpotce/.env` config (it is a base64 encoded string). Now identify the same string in the `LS_WEB_USER` ENV on the Hive in the `$HOME/tpotce/.env` config. Remove the string and restart SIEM Stack.<br>
Now you can safely delete the Sensor machine.

## Community Data Submission
SIEM Stack is provided in order to make it accessible to everyone interested in honeypots. By default, the captured data is submitted to a community backend. This community backend uses the data to feed [Sicherheitstacho](https://sicherheitstacho.eu).
...
1. Stop SIEM Stack services: `systemctl stop tpot`
...
```
4. Start SIEM Stack services: `systemctl start tpot`
...
As an Opt-In it is possible to share SIEM Stack data with 3rd party HPFEEDS brokers.
1. Follow the instructions [here](#community-data-submission) to stop the SIEM Stack services and open `~/tpotce/docker-compose.yml`.
...
Remote access to your host / SIEM Stack is possible with SSH (on **`tcp/64295`**) and some services and tools come with SIEM Stack to make some of your research tasks a lot easier.
<br><br>
...
## SIEM Stack Landing Page 
According to the [User Types](#user-types) you can open the SIEM Stack Landing Page from your browser via `https://<your.ip>:64297`:

- user: **[`<WEB_USER>`]**
- pass: **[password]**

![SIEM Stack-WebUI](doc/tpotwebui.png)
<br><br>

## Kibana Dashboard
On the SIEM Stack Landing Page just click on `Kibana` and you will be forwarded to Kibana. You can select from a large variety of dashboards and visualizations all tailored to the SIEM Stack supported honeypots.

![Dashbaord](doc/kibana_a.png)
<br><br>

## Attack Map
On the SIEM Stack Landing Page just click on `Attack Map` and you will be forwarded to the Attack Map. Since the Attack Map utilizes web sockets you may need to re-enter the `<WEB_USER>` credentials.

![AttackMap](doc/attackmap.png)
<br><br>

## Cyberchef
On the SIEM Stack Landing Page just click on `Cyberchef` and you will be forwarded to Cyberchef.

![Cyberchef](doc/cyberchef.png)
<br><br>

## Elasticvue
On the SIEM Stack Landing Page just click on `Elasticvue` and you will be forwarded to Elasticvue.

![Elasticvue](doc/elasticvue.png)
<br><br>

## Spiderfoot
On the SIEM Stack Landing Page just click on `Spiderfoot` and you will be forwarded to Spiderfoot.
...
## SIEM Stack Config File
SIEM Stack offers a configuration file providing variables not only for the docker services (i.e. honeypots and tools) but also for the docker compose environment. The configuration file is hidden in `~/tpoce/.env`. There is also an example file (`env.example`) which holds the default configuration.<br>
Before the first start run `~/tpotce/genuser.sh` or setup the `WEB_USER` manually as described [here](#add-users-to-nginx-SIEM Stack-webui). 

## Customize SIEM Stack Honeypots and Services

In `~/tpotce/compose` you will find everything you need to adjust the SIEM Stack Standard / Hive installation:
```
customizer.py
llm.yml
mac_win.yml
...
4. Stop SIEM Stack with `systemctl stop tpot`.
...
8. Start SIEM Stack with `systemctl start tpot`.
<br><br>

# Maintenance
SIEM Stack is designed to be low maintenance. Since almost everything is provided through docker images there is basically nothing you have to do but let it run. We will upgrade the docker images regularly to reduce the risks of compromise; however you should read this section closely.<br><br>
...
## General Updates
SIEM Stack security depends on the updates provided for the [supported Linux distro images](#choose-your-distro). Make sure to review the OS documentation and ensure updates are installed regularly by the OS. By default (`~/tpotce/.env`) `TPOT_PULL_POLICY=always` will ensure that at every SIEM Stack start docker will check for new docker images and download them before creating the containers.
<br><br>

## Update Script
SIEM Stack releases are offered through GitHub and can be pulled using `~/tpotce/update.sh`.<br> 
***If you made any relevant changes to the SIEM Stack config files make sure to create a backup first!***<br>
***Updates may have unforeseen consequences. Create a backup of the machine or the files most valuable to your work!***<br>

The update script will ...
 - ***mercilessly*** overwrite local changes to be in sync with the SIEM Stack master branch
 - create a full backup of the `~/tpotce` folder
 - update all files in `~/tpotce` to be in sync with the SIEM Stack master branch
 - restore your custom `ews.cfg` from `~/tpotce/data/ews/conf` and the SIEM Stack configuration (`~/tpotce/.env`).

## Daily Reboot
By default SIEM Stack will add a daily reboot including some cleaning up. You can adjust this line with `sudo crontab -e` 
```
#Ansible: SIEM Stack Daily Reboot
42 2 * * * bash -c 'systemctl stop tpot.service && docker container prune -f; docker image prune -f; docker volume prune -f; /usr/sbin/shutdown -r +1 "SIEM Stack Daily Reboot"'
```
...
### SIEM Stack Networking Fails
SIEM Stack is designed to only run on machines with a single NIC. SIEM Stack will try to grab the interface with the default route, however it is not guaranteed that this will always succeed. At best use SIEM Stack on machines with only a single NIC.

## Start SIEM Stack
The SIEM Stack service automatically starts and stops on each reboot (which occurs once on a daily basis as setup in `sudo crontab -l` during installation).
<br>
If you want to manually start the SIEM Stack service you can do so via `systemctl start tpot` and observe via `dpsw` the startup of the containers.
<br><br>

## Stop SIEM Stack
The SIEM Stack service automatically starts and stops on each reboot (which occurs once on a daily basis as setup in `sudo crontab -l` during installation).
<br>
If you want to manually stop the SIEM Stack service you can do so via `systemctl stop tpot` and observe via `dpsw` the shutdown of the containers.
<br><br>

## SIEM Stack Data Folder
All persistent log files from the honeypots, tools and SIEM Stack related services are stored in `~/tpotce/data`. This includes collected artifacts which are not transmitted to the Elastic Stack.
<br><br>

## Log Persistence
All log data is stored in the [SIEM Stack Data Folder](#SIEM Stack-data-folder) and will be persisted for the number of cycles set for `TPOT_PERSISTENCE_CYCLES=<1-999>` in the SIEM Stack configuration file `~/tpotce/.env`. It defaults to 30.
<br>
...
## Factory Reset
All log data stored in the [SIEM Stack Data Folder](#SIEM Stack-data-folder) (except for Elasticsearch indices, of course) can be erased by running `clean.sh`.
Sometimes things might break beyond repair and it has never been easier to reset a SIEM Stack to factory defaults (make sure to enter `cd ~/tpotce`).
1. Stop SIEM Stack using `systemctl stop tpot`.
...
4. Reset SIEM Stack to the last fetched commit:
```
cd ~/tpotce/
git reset --hard
```
...
## Add Users to Nginx (SIEM Stack WebUI)
Nginx (SIEM Stack WebUI) allows you to add as many `<WEB_USER>` accounts as you want (according to the [User Types](#user-types)).<br>
...
For the changes to take effect you need to restart SIEM Stack using `systemctl stop tpot` and `systemctl start tpot` or `sudo reboot`.
<br><br>

## Import and Export Kibana Objects
Some SIEM Stack updates will require you to update the Kibana objects. Either to support new honeypots or to improve existing dashboards or visualizations. Make sure to ***export*** first so you do not loose any of your adjustments.
...
# Troubleshooting
Generally SIEM Stack is offered ***as is*** without any commitment regarding support. Issues and discussions can be opened, but be prepared to include basic necessary info, so the community is able to help.
<br><br>
...
While SIEM Stack keeps trying to restart the services / containers run `docker logs -f <container_name>` (either `logstash` or `elasticsearch`) and check if there are any warnings or failures involving RAM.

Storage failures can be identified easier via `htop`. 
<br><br>

# Contact
SIEM Stack is provided ***as is*** open source ***without*** any commitment regarding support ([see the disclaimer](#disclaimer)).
...
# Licenses
The software that SIEM Stack is built on uses the following licenses.
<br>GPLv2:
...
# Credits
Without open source and the development community we are proud to be a part of, SIEM Stack would not have been possible! Our thanks are extended but not limited to the following people and organizations:
<br><br>
...
***"[...] I highly recommend SIEM Stack which is ... it's not exactly a swiss army knife .. it's more like a swiss army soldier, equipped with a swiss army knife. Inside a tank. A swiss tank. [...]"***
<br><br>
